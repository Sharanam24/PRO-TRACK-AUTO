...
export async function getGroupEvaluations(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
        ...
        const evaluations = await query(
            `SELECT p.*, s_tor.full_name as evaluator_name, s_tee.full_name as evaluatee_name
             FROM peer_evaluations p
             JOIN student_whitelist s_tor ON p.evaluator_id = s_tor.id
             JOIN student_whitelist s_tee ON p.evaluatee_id = s_tee.id
             WHERE p.group_id = $1`,
            [group_id]
        );
