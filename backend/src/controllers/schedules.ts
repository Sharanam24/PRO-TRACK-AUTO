...
export async function createOrUpdateSchedule(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
        ...
        // Notify group members of the new/updated schedule (Requirement 5.2)
        try {
            ...
            await Promise.all(
                members.map((m) =>
                    createNotification({
                        ...
                        title: `Schedule Updated: ${phase as string}`,
                        message: `Presentation for ${phase as string} scheduled at ${presentation_time as string}${venue ? ` in ${venue as string}` : ''}.`,
                        type: 'schedule',
                        priority: 'medium',
                    })
                )
            ...
            console.error('Schedule notification error:', notifErr);
        }
