...
import { AlertCircle, Plus, CheckCircle, TrendingUp, Users, BookOpen, Zap, Hash, GraduationCap, Copy, CheckCheck, PenTool, FileDown } from 'lucide-react';
import { generateStudentReport } from '../../lib/pdfService';
                        ...
                        <button
                            onClick={() => {
                                ...
                                generateStudentReport(
                                    {
                                        name: user?.email ?? '',
                                        prn: (user as any)?.prn_no ?? 'unknown',
                                        roll_no: (user as any)?.roll_no ?? '',
                                    },
                                    {
                                        group_id: firstGroup?.group_id ?? '',
                                        group_name: firstGroup?.group_name ?? 'N/A',
                                        members: [],
                                        guide_name: firstGroup?.guide_email ?? undefined,
                                    },
                                    []
                                );
                            }}
                            disabled={groups.length === 0}
                            className="flex items-center gap-1.5 px-3 py-2 bg-purple-500/20 border border-purple-500/30 text-purple-300 text-xs font-bold rounded-xl hover:bg-purple-500/30 disabled:opacity-40 transition-colors"
                        >
                            <FileDown size={13} /> My Performance Report
                        </button>
                    </div>
                </div>
            </div>
