...
import { CheckCircle, Clock, Trophy, Star, Mic, MicOff, MessageSquare, Send, FileDown } from 'lucide-react';
import { useAuthStore } from '../../store/authStore';
import { api } from '../../lib/apiClient';
import { generateEvaluationSheet } from '../../lib/pdfService';
                            ...
                            <button
                                onClick={() => {
                                    generateEvaluationSheet(
                                        {
                                            phase: 'FINAL',
                                            total_marks: parseFloat(totalScore),
                                            rubric_scores: scores,
                                            remarks: generalFeedback,
                                        },
                                        { criteria: evaluationCriteria.map(c => ({ id: c.criteria, label: c.criteria, max_marks: c.maxScore })) }
                                    );
                                }}
                                className="px-4 py-2.5 text-sm border border-indigo-500/30 text-indigo-300 rounded-xl hover:bg-indigo-500/10 font-semibold transition-all flex items-center gap-1.5"
                            >
